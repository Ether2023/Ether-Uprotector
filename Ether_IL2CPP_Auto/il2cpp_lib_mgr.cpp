...
namespace il2cpp_lib_mgr
{
	void proc_lib(const char* p) {
		...
		for (auto s : f_in_vm) {
			...
			// MetadataCache.cpp
			if (cpp_mgr::find_str_in_file(lines, L"IL2CPP_ASSERT(s_GlobalMetadataHeader->sanity", 0) != -1) {
				cout << "done" << endl;
				CopyFileA(s.c_str(), (s + EXT_OZ_BACKUP).c_str(), FALSE);
				// make backup
				succ = true;
				cout << "[il2cpp_lib_mgr] " << "processing MetadataCache.cpp...\n";
				cpp_mgr::proc_cpp((s + EXT_OZ_BACKUP).c_str(), s.c_str(), "./oz_scripts/mdc.ozs");
				cout << "done" << endl;
				break;
			}
	...
	bool find_proc_metadatahead(const char* p) {
		...
		for (auto s : fs) {
			if (s.find(EXT_OZ_BACKUP) != string::npos) {
				cout << "[il2cpp_lib_mgr] " << "already installed. please uninstall first." << " path:" << sp << endl;
				return false;
			}
		}
		...
		for (auto s : fs) {
			...
			if (cpp_mgr::find_str_in_file(lines, L"typedef struct Il2CppGlobalMetadataHeader", 0) != -1) {
				...
				cpp_mgr::proc_cpp((s + EXT_OZ_BACKUP).c_str(), s.c_str(), "./oz_scripts/metadataheader.ozs");
				return true;
			}
